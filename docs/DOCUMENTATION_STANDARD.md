---
# This standard is itself an anchored doc — it obeys its own I.8 and II.9.
covers_files:
  - .gitattributes
...
(I.1–I.10) governs *authoring*. Part II (II.1–II.9) governs the *verification
...
> yet been committed and validated against this repository's tree. Per II.9 the
...
The authoring directives fall into three themes: anchoring (I.1–I.3), content (I.4–I.7), and verifiability (I.8–I.10).

## I.1 — Reference symbols, never line numbers.
...
## I.2 — Cite the smallest stable anchor available.
...
## I.3 — Deleting a reference requires proving absence, not just a failed lookup.
...
## I.4 — Describe contracts and intent, not transcribed code.
...
## I.5 — Every number and default has exactly one source of truth.
...
## I.6 — Document deliberate-but-surprising code loudly.
...
## I.7 — Model non-deterministic behavior as ranges and conditions, not single instants.
...
## I.8 — Lead each doc with a machine-readable anchor block.
...
block's precise meaning and the rules for advancing its marker are defined in Part II.
...
## I.9 — Make every claim falsifiable.
...
## I.10 — End each doc with a verification recipe.
...
covered path. For anchored docs this recipe is executed automatically by the Part II tooling (`check_doc_staleness.sh`); the recipe in prose is the human-readable form of the
...
# Part II — The Verification Marker
...
## II.1 — Redefine what the marker means (so it cannot lie)
...
clause that depends on a person, and II.3 and II.7 force it to leave evidence. The
...
## II.2 — The staleness check MUST be symbol-granular, and the driver MUST resolve every anchor
...
- **The diff driver MUST resolve every symbol class the docs anchor on**, not only a
  ...
  own tree per II.6 before trusting it):
...
- **`covers_symbols` entries MUST be `{symbol, file}` pairs.** A bare file (no symbol) is
  ...
  (I.1).
- **Symbol names MUST be unambiguous within their file** (see also II.8). An ambiguous `-L`
...
- unresolvable at HEAD → renamed, moved, or deleted → **STALE** (I.3 applies: confirm
  which, from the repo root, before editing the reference)
- empty → unchanged → pass

A reference implementation — the same configuration its own prose prescribes (II.9). **Tooling
...
# Deliberately does NOT compare markers across docs (see II.4).
...
# These are representative; validate against the tree per II.6 (and Part III.2 for .svelte/.ts).
...
## II.3 — Advancing the marker MUST be the output of a re-read

This is the only moment a person is trusted, so it is the only place the lie can survive II.1.
...
## II.4 — Divergence is the healthy state; uniformity MUST NOT be enforced

CI MUST validate, per doc, the marker's **format** (a full 40-hex SHA) and **reachability** (an
ancestor of HEAD), and the **II.2 staleness check**.
...
docs share one SHA" invariant rewards synchronized bumping — the precise hollow move II.3
...
## II.5 — Required edge-case handling

| Situation | Required behavior |
|-----------|-------------------|
| Covered symbol renamed | STALE → re-audit, fix prose **and** `covers_symbols`, then advance per II.3 / II.7. Confirm rename vs. deletion from the repo root first (I.3). |
| Covered symbol moved to another file | STALE → update the `file` field in `covers_symbols`, then advance per II.3 / II.7. |
| Symbol name ambiguous within its file | Defect in the doc — qualify or refactor the anchor (II.8). Never fall back to a line range. |
...
| Constant/env-var anchor returns rc=128 | The funcname driver is not extended per II.2 — fix the driver, do not abandon the anchor. |
...
## II.6 — Empirical findings about resolution MUST be propagated to the normative text
...
- **A II.3 affirmation MUST NOT be issued for a doc while a known, resolution-affecting finding
...
## II.7 — Mutating `covers_symbols` is a reconciliation act

Editing `covers_symbols` — adding an anchor, swapping one for another, or removing one — is
itself a reconciliation act governed by II.3, **not** bookkeeping that leaves the marker
...
established for each added or swapped-in anchor — the same auditable trace II.3 requires.
...
**Removing an anchor.** Per I.3, first confirm from the repo root whether the symbol
...
## II.8 — Prefer a qualified unambiguous anchor over abandonment
...
## II.9 — The standard applies to itself; "verbatim" is not a defense
...
tooling's real requirements (II.6), and the standard's own anchor block, affirmations, and
...
substitute with different semantics is a silent prerequisite of the kind II.6 forbids.
...
- The **extended funcname drivers** in II.2 MUST be present either by invocation (`git -c …`, as
  the script does) or a repo-tracked config include, and MUST be validated against this tree per
  II.6 before any marker is trusted.
...
- **Frontend `.svelte` / `.ts` anchors.** Several docs anchor frontend symbols
  ...
  less reliably than Python `def`/`class`/constant lines. For these, follow II.8: prefer a
...
in a file it covers goes entirely undocumented, because the doc never claimed that symbol. II.7
and II.8 guard coverage at the edges of existing docs; nothing in Part II detects **under-coverage**
...
I.5 binds each individual magic value to one symbol. Its corpus-level corollary: **aggregate
...
## III.6 — Adoption checklist (required before any self-affirmation; discharges II.9)

Until every box is checked **in the repository**, this standard is stale against its own
implementation and none of its self-affirmations stand (II.9):

- [ ] Commit `tools/check_doc_staleness.sh`.
- [ ] Commit `.gitattributes` mapping `*.py`, `*.ts`, `*.svelte` to extended funcname drivers (II.2).
...
- [ ] Wire the check into CI and/or a pre-push hook (II.4), plus the upstream-merge trigger (III.2).
...
*meaning* was narrowed to something git proves unaided (II.1); its *granularity* was matched to the
doc's own symbol anchors (II.2) so it fires only on real drift; the one irreducibly human moment was
forced to coincide with the prose edit or leave an auditable affirmation (II.3, II.7); and
uniformity, the structural incentive to fake that moment, was banned (II.4). The remaining clauses
extend the same obligation **up one level** (II.6, II.9) and **sideways** (II.7), with II.8
